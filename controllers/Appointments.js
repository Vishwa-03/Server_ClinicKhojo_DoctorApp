...
exports.getAppointmentsByType = async (req, res) => {
  ...
  try {
    const appointments = await Appointment.find({type:type,doctor:doctor._id });
      // .populate('patient', 'name profilePicture'); // Populate user data
...
exports.createAppointment = async (req, res) => {
  try {
    ...
    // B. Doctor Information Embedded (Optional)
    // const newAppointment = new Appointment({
    //   type,
    //   patient,
    //   block,
    ...
    return res.status(400).json({ message: 'Error creating appointment', error: err.message }); // Provide an informative error message
  }
};
