...
// router.post("/auth/register/management", async (req, res) => {
//   try {
//     // ... existing logic for creating a management profile with robust validation (refer to previous examples)
//   } catch (err) {
...
// 2. Login for management personnel (implement using JWT)
router.post("/auth/register/management", async (req, res) => {
  try {
...
router.get("/management/email/:email", async (req, res) => {
...
router.post("/createHospitals", async (req, res) => {
  try {
...
router.post("/management/addDoctors", async (req, res) => {
  ...
  try {
    ...
    const doctor2 = await Doctor.find({
      registrationNumber: doctorRegistrationNumber,
    });
    if (!doctor || !doctor2) {
    ...
    if (hospital&&hospital.doctorsName?.some(doctor => doctor.name === doctorName)) {
      return res
        .status(409)
...
router.delete("/doctors/:doctorId", async (req, res) => {
  ...
  try {
    ...
    // Ensure authorized user has permission to delete
    if (!isAuthorizedToDeleteDoctor(req.user, doctorId)) {
      // Implement this check
      return res.status(403).send("Unauthorized to delete doctor");
    }
